...
BCG.Level.Membership <- function(df.metric.membership
  ...
  df.merge <- df.merge %>%
    dplyr::mutate(MEMBERSHIP = dplyr::case_when(EXC_RULE == "FLIPMINMAX" ~ - MEMBERSHIP,
                                                .default = MEMBERSHIP))
  # CHANGE BACK LATER IN CODE with abs() in next step (Summarize)

  ## EXC_RULE, MIN012MAX----
  df_er_min012max <- dplyr::filter(df.merge, EXC_RULE == "MIN012MAX")
  # group
  # calculate for MIN each Rule Type then MAX
  # Will get lots of warnings, SampIDs without alt 1 or alt 2 rules

  # MIN of each Rule_Type
  suppressWarnings(
  df_er_min012_min_calc <- dplyr::summarize(dplyr::group_by(df_er_min012max,
                                                           SAMPLEID,
                                                           INDEX_NAME,
                                                           INDEX_CLASS,
                                                           LEVEL),
                    ...
                    MembCalc_Rule2_min = min(MEMBERSHIP[RULE_TYPE == "RULE2"],
                                             na.rm = TRUE),
                    MembCalc_Rule1_min = min(MEMBERSHIP[RULE_TYPE == "RULE1"],
                                             na.rm = TRUE),
                    MembCalc_Rule0_min = min(MEMBERSHIP[RULE_TYPE == "RULE0"],
                                             na.rm = TRUE)) %>%
    # Inf and -Inf to NA
    dplyr::mutate(MembCalc_Rule2_min = dplyr::case_when(
        MembCalc_Rule2_min == Inf ~ NA_real_,
        MembCalc_Rule2_min == -Inf ~ NA_real_,
        .default = MembCalc_Rule2_min),
      MembCalc_Rule1_min = dplyr::case_when(
        MembCalc_Rule1_min == Inf ~ NA_real_,
        MembCalc_Rule1_min == -Inf ~ NA_real_,
        .default = MembCalc_Rule1_min),
      MembCalc_Rule0_min = dplyr::case_when(
        MembCalc_Rule0_min == Inf ~ NA_real_,
        MembCalc_Rule0_min == -Inf ~ NA_real_,
        .default = MembCalc_Rule0_min))
  )## suppressWarnings

  # max of Rule_Types
  suppressWarnings(
  df_er_min012max_max_calc <- dplyr::summarize(
    dplyr::group_by(df_er_min012_min_calc,
                    SAMPLEID,
                    INDEX_NAME,
                    INDEX_CLASS,
                    LEVEL),
    .groups = "drop_last",
    MEMBERSHIP = max(MembCalc_Rule2_min,
                     MembCalc_Rule1_min,
                     MembCalc_Rule0_min,
                     na.rm = TRUE)) %>%
    dplyr::mutate(RULE_TYPE = "RULE0")
  ...
  #
  # Update df.merge
  # Remove EXC_RULE == "MIN012MAX"
  df.merge <- dplyr::filter(df.merge, EXC_RULE != "MIN012MAX" | is.na(EXC_RULE))
  # Add new memberships back to df.merge
  df.merge <- dplyr::bind_rows(df.merge, df_er_min012max_max_calc)
  # should be reduced to 1 record instead of multiple metrics
