# Calculate BCG Panel, marinw

function() {
  sidebarLayout(
    sidebarPanel(
       h2("Calculate BCG Model, MariNW")
       ...
       , p(textOutput("fn_input_display_bcg_marinw"))
       
       , h4("B. Define Community (for metrics).")
       , selectInput("si_community_bcg_marinw"
                     , label = "Community"
       ...
       , checkboxInput("ExclTaxa_bcg_marinw"
       ...
       , uiOutput("UI_bcg_marinw_modelexp_user_col_precip")
       , uiOutput("UI_bcg_marinw_modelexp_user_col_wshedarea_km2")
       , uiOutput("UI_bcg_marinw_modelexp_user_col_elev")
       , uiOutput("UI_bcg_marinw_modelexp_user_col_slope")
       
       , h4("F. Run Calculations")
       , p("This button will calculate metrics values, metric memberships
           , level membership, and level assignment.")
       , useShinyjs()
       , shinyjs::disabled(shinyBS::bsButton("b_calc_bcg_marinw"
                                             , label = "Run Calculations"))
       
       , h4("G. Download Results")
       , p("All input and output files will be available in a single zip file.")
       , shinyjs::disabled(downloadButton("b_download_bcg_marinw"
                                          , "Download Results"))
        )## sidebarPanel ~ END
    , mainPanel(
        tabsetPanel(type = "tabs"
                    , tabPanel(title = "Calc_BCG_MariNW_About"
                               ,includeHTML(file.path("www"
                                                      , "rmd_html"
                                            , "ShinyHTML_Calc_BCG_marinw_1About.html"))
                               )
                    , tabPanel(title = "Calc_BCG_MariNW_Input"
                               ,includeHTML(file.path("www"
                                                      , "rmd_html"
                                            , "ShinyHTML_Calc_BCG_marinw_2Input.html"))
                               )
                    , tabPanel(title = "Calc_BCG_MariNW_Output"
                               ,includeHTML(file.path("www"
                                                      , "rmd_html"
                                          , "ShinyHTML_Calc_BCG_marinw_3Output.html"))
                               )
                    )## tabsetPanel ~ END
