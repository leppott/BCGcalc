...
function() {
  sidebarLayout(
    sidebarPanel(h2("2. File Builder")
            ...
            , p("Select user file Taxa ID column.")
            , uiOutput("UI_taxatrans_user_col_taxaid")
            , p("Select any user file columns to drop from output.")
            , p("All other columns will be retained.")
            , uiOutput("UI_taxatrans_user_col_drop")
            
            , h4("2.D. Summarize Duplicate Sample Taxa")
