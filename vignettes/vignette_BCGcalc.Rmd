...
installed using the following lines of code. It is necssary to install the 
`devtools` package.
...
*	FFG, HABIT, LIFE_CYCLE, TOLVAL, THERMAL_INDICATOR
    ...
    +	LIFE_CYLCE
        ...
        - Life cyle designations need to come from the user (the BCG workgroup did 
        not attempt to reach regional consensus/reconcile differences across entities).
...
NHDPlus v2 flowline slope but users can also base their gradient designations on 
...
metrics comparable across the 500-count samples.   Samples with leass an 400 organisms 
...
If you decide to run the example below, the following five files will appear in 
...
(e.g., Washinton is 18891111).  These values can be found on Wikipedia on the 
...
2. Run the ‘normal’ `metrics.values` R code for to generate an output that i
ncludes the full set of metrics, then examine the results and run additional 
